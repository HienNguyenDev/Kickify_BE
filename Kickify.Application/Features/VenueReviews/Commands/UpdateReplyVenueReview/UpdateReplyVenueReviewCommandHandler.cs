...
public class UpdateReplyVenueReviewCommandHandler
    : ICommandHandler<UpdateReplyVenueReviewCommand, ReplyVenueReviewResponse>
{
    private readonly IVenueReviewRepository _venueReviewRepository;
    private readonly IUserRepository _userRepository;
    private readonly IUserContext _userContext;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ILogger<UpdateReplyVenueReviewCommandHandler> _logger;

    public UpdateReplyVenueReviewCommandHandler(
        IVenueReviewRepository venueReviewRepository,
        IUserRepository userRepository,
        IUserContext userContext,
        IUnitOfWork unitOfWork,
        ILogger<UpdateReplyVenueReviewCommandHandler> logger)
    {
    ...
    public async Task<Result<ReplyVenueReviewResponse>> Handle(
        UpdateReplyVenueReviewCommand request, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(review.OwnerResponse))
            return Result.Failure<ReplyVenueReviewResponse>(VenueReviewErrors.NoReplyToUpdate);
        ...
        _logger.LogInformation(
            "Venue owner {OwnerId} updated reply on review {ReviewId} for venue {VenueId}",
